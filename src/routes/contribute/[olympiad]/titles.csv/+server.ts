...
export const GET: RequestHandler = async ({ params, locals }) => {
	...
	const rows = await db
		.select({ year: years.year, number: problems.number, title: problems.title })
		.from(problems)
	...
	const lines = ['year,number,title'];
	for (const row of rows) {
		lines.push(
			[String(row.year), csvField(row.number), csvField(row.title ?? '')].join(
				','
			)
		);
	}
